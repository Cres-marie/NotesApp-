...
class SmolleyNotification{
  ...
  static Future dailyNotification({required String title, required String body, var payload,required DateTime time, required FlutterLocalNotificationsPlugin fn}) async{
    ...
    var notification = NotificationDetails(android: androidnotificationdetails);
    tz.initializeTimeZones();
    var today = tz.TZDateTime(tz.local, DateTime.now().year,DateTime.now().month,DateTime.now().day, time.hour, time.minute);
  ...
  ShowNotification(title: 'Hey there! Do you know what time it is?', body: title, time: time, fn: fn);
  }
  static Future<void> checkScheduledNotifications(FlutterLocalNotificationsPlugin fn) async {
